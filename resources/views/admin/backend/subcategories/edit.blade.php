...
@section('content')
    <div class="table-container shadow py-3 px-4 mb-5 bg-white" style="border: 2px solid #ccc;">
        <form action="{{ url('ecommerce/admin/subcategories/' . $subcategory->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
            ...
            <div class="row">
                <div class="col-sm-6 mb-3">
                    ...
                    @if ($subcategory->image)
                        <img class="m-1 custom-thumbnail" src="{{ Storage::url($subcategory->image) }}" width="100" height="100"
                            id="imagePreview">
                    @else
            ...
            <div class="row">
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <select name="status" class="form-select" id="status">
                        <option value="0" @selected(old('status', $subcategory->status) == 0)>Visible</option>
                        <option value="1" @selected(old('status', $subcategory->status) == 1)>Hidden</option>
                    </select>
                    @error('status')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                ...
                <div class="col-sm-3 mb-3">
                    ...
                    <select name="is_popular" class="form-select" id="is_popular">
                        <option value="0" @selected(old('is_popular', $subcategory->is_popular) == 0)>No</option>
                        <option value="1" @selected(old('is_popular', $subcategory->is_popular) == 1)>Yes</option>
                    </select>
                    @error('is_popular')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
    ...
    <script>
        ...
        imageInput.addEventListener('change', function() {
            ...
            if (file) {
                ...
                imagePreview.src = "{{ Storage::url($subcategory->image) }}";
            }
        });
