        ...
        <form action="{{ url("ecommerce/admin/brands/$brand->id") }}" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-12 mb-3">
                    ...
                    @if ($brand->image)
                        <img class="m-1 custom-thumbnail" src="{{ Storage::url($brand->image) }}" width="100" height="100"
                            id="imagePreview">
                    @else
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="status" class="form-select" id="status">
                        <option value="0" @selected(old('status', $brand->status) == 0)>Visible</option>
                        <option value="1" @selected(old('status', $brand->status) == 1)>Hidden</option>
                    </select>
                    @error('status')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
    ...
    <script>
        ...
        imageInput.addEventListener('change', function() {
            ...
            if (file) {
                ...
                imagePreview.src = "{{ Storage::url($brand->image) }}";
            }
        });
